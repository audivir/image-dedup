    ...
    while let Ok(progress_data) = progress_receiver.recv() {
        ...
        if latest_id != Some(progress_data.current_stage_idx) {
            ...
            if progress_data.current_stage_idx == 0 {
                ...
                pb = get_progress_loading_saving_cache(
                    progress_data.sstage.check_if_loading_cache(),
                );
        ...
        if progress_data.sstage == CurrentStage::CollectingFiles
...
pub(crate) fn get_progress_message(progress_data: &ProgressData) -> String {
    match progress_data.sstage {
        ...
        CurrentStage::SimilarImagesHidingHardLinks | CurrentStage::SimilarVideosHidingHardLinks => {
            "Hiding duplicates with hardlinks"
        }
        CurrentStage::SimilarVideosCreatingThumbnails => "Creating video thumbnails",

        _ => {
            unreachable!(
                "This stages(caches, initial files scanning) should be handled somewhere else"
            )
        }
    }
    .to_string()
